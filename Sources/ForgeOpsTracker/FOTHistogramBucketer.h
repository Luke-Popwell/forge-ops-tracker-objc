 ...
 * FOTPerformanceFlusher uses to accumulate an approximate distribution (not just count/sum/max)
 ...
 *
 * +boundariesMs is duplicated on the server side, in app/services/histogram_percentile.rb. Change
 ...
 */
@interface FOTHistogramBucketer : NSObject

/** The fixed latency-range upper bounds, in milliseconds. */
+ (NSArray<NSNumber *> *)boundariesMs;

/**
 * The label of the smallest boundary durationMs fits under, or @"inf" for anything larger than the
 * largest boundary. An NSString, not a number: this travels as a JSON object key once flushed, and
